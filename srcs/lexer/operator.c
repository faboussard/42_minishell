...
/*   Updated: 2024/05/27 10:27:38 by faboussa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	define_operator_token(t_token_list *new_token, const char *string,
...
bool	is_redirect_token(t_token_list *token)
{
	...
	return (token->e_operator == INPUT_REDIRECT
		|| token->e_operator == OUTPUT_REDIRECT || token->e_operator == HERE_DOC
...
bool	is_redirect_token_or_pipe(t_token_list *token)
{
	...
	return (token->e_operator == INPUT_REDIRECT
		|| token->e_operator == OUTPUT_REDIRECT || token->e_operator == HERE_DOC
		|| token->e_operator == APPEND || token->e_operator == PIPE);
}
