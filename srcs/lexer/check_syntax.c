...
//REVOIR pour la question duhere doc on execute ou on lqnce le message avant ??
...
size_t check_quotes(t_minishell *minishell)
{
	...
	while (iterator != NULL)
	{
		...
		{
			quote_double++;
			first_odd_quote = get_first_odd_quote(quote_double, first_odd_quote, DOUBLE_QUOTE);
		}
		if (iterator->e_operator == SINGLE_QUOTE)
		{
			quote_simple++;
			first_odd_quote = get_first_odd_quote(quote_simple, first_odd_quote, SINGLE_QUOTE);
		}
		iterator = iterator->next;
	}
