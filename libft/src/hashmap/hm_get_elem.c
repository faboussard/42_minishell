...
/// @brief 			returns the t_hashmap_node* (a) in the hashmap that has
...
/// @return 		returns the t_hashmap_node* (a) in the hashmap that has
///						a->target == target, returns NULL if it doesn't exist
t_hashmap_node	*ft_hm_get_elem(t_hashmap map, char *target)
{
	...
	while (cursor != NULL
		&& ft_strcmp(((t_hashmap_node *)cursor->content)->target, target)
		!= 0)
		cursor = cursor->next;
	if (cursor == NULL)
		return (NULL);
